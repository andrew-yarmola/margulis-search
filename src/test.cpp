...
// char code[] = "010111101000101000011000100100001111101010001000011101101000001101001101111110010000"; 
...
//    printf("%c\n", *dir);
  ...
  double xx_UB = e_re_perp_x_UB(x);
  double xy_UB = e_re_perp_x_UB(y);
  double yx_UB = e_re_perp_y_UB(x, coshP, sinhP);
  double yy_UB = e_re_perp_y_UB(y, coshP, sinhP);

  printf("Margulis xy between %f and %f\n", m_xy_lower, m_xy_upper);
  printf("Exp 2t xy between %f and %f\n", t_xy_lower, t_xy_upper);
  printf("Re conj uppers xx %f, xy %f, yx %f, yy %f\n", xx_UB, xy_UB, yx_UB, yy_UB);
