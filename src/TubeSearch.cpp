...
namespace TubeSearchImpl {
  ...
  inline bool pos_neq(double a, double b) {
	...
	Word product(const Word& lhs, const Word& rhs, const Params<Complex>& params)
	{
		Word result;
...
//   fprintf(stderr,"push_word(%s)\n", word.name.c_str());
    bool pushed = false;
  
    if (pos_less(word.four_cosh_x_x, absUB(params.cosh2dx * 5))   ||  
        pos_less(word.four_cosh_y_x, absUB(params.cosh2dy * 5))   ||
        pos_less(word.four_cosh_x_y, absUB(params.coshdxdy * 5))  ||
        pos_less(word.four_cosh_y_x, absUB(params.coshdxdy * 5))  ||
        pos_less(word.cosh_move, absUB(params.coshmu * 1.5))    ||
        pos_less(word.jorg_x, 1.5)                                ||
        pos_less(word.jorg_y, 1.5))
    {
...
//    fprintf(stderr, "First word %s, Second word %s\n", it->name.c_str(), word.name.c_str());
          ...
          pos_less(pair.four_cosh_marg, absUB(params.coshmu * 5))    ||
          pos_less(pair.cosh_two_ortho, absUB(params.coshdxdy * 1.5)))
      {
        fprintf(stderr,"push WordPair(%s,%s)\n", pair.first.name.c_str(), pair.second.name.c_str());
				pairs.push(pair);
        pushed = true;
      }
		}
...
#define POW_DEPTH 5
	...
	{
    int loop_count = 0;
		while (pairs.size() > 0 && loop_count < SEARCH_DEPTH) {
      Word w(words.top()); 
      fprintf(stderr, "Smallest wor %s\n", w.name.c_str());
      if (y_power(w.name) > 0 && x_power(w.name) > 0 &&
          (pos_less(w.four_cosh_x_x, absLB(params.cosh2dx * 4))   ||  
           pos_less(w.four_cosh_y_x, absLB(params.cosh2dy * 4))   ||
           pos_less(w.four_cosh_x_y, absLB(params.coshdxdy * 4))  ||
           pos_less(w.four_cosh_y_x, absLB(params.coshdxdy * 4))  ||
           pos_less(w.cosh_move, absLB(params.coshmu))          ||
           pos_less(w.jorg_x, 1.0)                                ||
           pos_less(w.jorg_y, 1.0)))
      {
      ...
      if (w.name[0] == 'y' || w.name[0] == 'Y') {
        int x_pow = 0;
        while(push_word(product(x_word, w, params)) && x_pow < POW_DEPTH) {
          x_pow++;
        }
        int X_pow = 0;
        while(push_word(product(X_word, w, params)) && X_pow < POW_DEPTH) {
        ...
        int y_pow = 0;
        while(push_word(product(y_word, w, params)) && y_pow < POW_DEPTH) {
          y_pow++;
        }
        int Y_pow = 0;
        while(push_word(product(Y_word, w, params)) && Y_pow < POW_DEPTH) {
      ...
      fprintf(stderr, "Smallest pair (%s, %s)\n", p.first.name.c_str(), p.second.name.c_str());
			if (pos_less(p.jorgensen_words, 1.0)                     ||
          pos_less(p.four_cosh_marg, absLB(params.coshmu * 4)) ||
      ...
      push_word(product(inverse(p.first, params), p.second, params));
      push_word(product(p.first, inverse(p.second, params), params));
      push_word(product(p.first, p.second, params));
		}
    Word w;
    w.name = "";
  ...
  int num_words, int max_length, vector<string> relators)
{
  TubeSearchImpl::TubeSearch search(center);
	...
	while (num_words > int(search.words.size()) || (-num_words > int(search.words.size()) && !search.found_good_pair())) {
    fprintf(stderr, "While loop: words size %d\n", (int) search.words.size());
		TubeSearchImpl::WordPair p = search.find_pair();
		if (p.first.name.length() > max_length || p.second.name.length() > max_length) {
